...
# Ответ 1. Кросс-валидация для градиентного бустинга с 30 деревьями проводилась 0:00:22 и показатель метрики качества
# равен 0.69 2. При увелечения числа деревьев показатель скоринга увеличивается до 0.7, но также увеличивается время
# работы, поэтому можно использовать большее количество деревьев Чтобы ускорить работу алгоритма можно отфильтровать
# часть данных, убрав выбросы. А также поиграть с параметрами классификатора.
...
# Ответ Наилучший параметр регуляризации с равен 0.0001. Скоринг при нём равен 0.72. Время 0:00:01.99. Работае
# заметно быстрее. Градиентный бустинг даёт такой результат на количестве n_estimators большем 100 (не проверял
# больше 45) Думаю, разница в том, что зависимость в данных близка к линейной, поэтому линейной регресси достаточно,
# чтобы хорошо её найти.
...
# Ответ
# После добавления "мешка слов" скоринг увечился до 0.75 при c = 0.001.
